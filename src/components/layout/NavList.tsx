...
const ListItem = ({ pathName, route, label }: { pathName: string; route: string; label: string }) => (
	<Link
		href={route as RouteHref}
		className={clsx(
			'py-3 text-lg transition-colors md:py-2',
			isRouteActive(pathName, route) ? 'text-primary-solid' : 'text-canvas-text hover:text-canvas-text-contrast',
		)}
	>
		{label}
	</Link>
...
// 	<li className={clsx('p-2', isRouteActive(pathName, route) ? 'text-gray-400' : 'text-foreground')}>
...
export default function NavList() {
	...
	const navItems = mainNavigation.filter(
		(item) =>
			(item.displayWenUnauthenticated && status === 'unauthenticated') ||
			(item.displayWenAuthenticated && status === 'authenticated'),
	)
	...
	return (
		<div className="relative">
			<div className="hidden flex-row items-center gap-2 sm:flex sm:gap-4">
					...
					<NavigationMenu.Item asChild>
						<div className="flex items-stretch">
							...
							<NavigationMenu.Content onPointerEnter={(event) => event.preventDefault()} onPointerLeave={(event) => event.preventDefault()} className="absolute left-0 top-[44px] left-100 z-30 animate-in fade-in duration-300">
								<div className="w-auto sm:min-w-[224px] flex-auto overflow-hidden rounded-md bg-background 2xl:text-base leading-6 shadow-lg ring-1 ring-gray-900/5">
