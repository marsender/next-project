...
interface UserStateItem {
	...
	state_value: any // The actual value of the state. 'any' is used here as it can be flexible (string, number, object, etc.).
}

// The value to set or get can be a string or a json object
export const stateService = {
	async getUserState<T = any>(userId: string, key: string): Promise<T | null> {
		try {
	...
	async setUserState<T = any>(userId: string, key: string, value: T): Promise<void> {
		try {
